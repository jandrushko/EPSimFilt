...
def generate_methods_text(settings):
    ...
    # Signal generation
    if settings.get('mep_type'):
        morphology = settings['mep_type']
        ...
        methods.append(f"{morphology} motor evoked potentials were simulated using the MEP Filter Testing Tool v1.4.0 (Andrushko, 2024) ")
    ...
    if settings.get('filter_configs'):
        ...
        methods.append("Results are reported as mean ± standard deviation across all iterations.")
