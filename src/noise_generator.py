"""
Noise Generator
Generate various types of noise commonly found in EP recordings.
"""
...
class NoiseGenerator:
    """Generate realistic noise for EP recordings."""
    ...
    def add_emg_noise(self,
        ...
        """
        Add EMG-like noise with realistic 1/f spectral characteristics.
        
        Real EMG has more power at lower frequencies, decreasing at higher frequencies.
        Uses pink noise (1/f) spectrum for physiological realism.
        ...
        --------
        noisy_signal : np.ndarray
            Signal with added realistic EMG noise
        """
        # Generate white noise
        white = np.random.randn(len(signal))
        
        # Apply 1/f spectral shaping (pink noise characteristics)
        # This gives more power at low frequencies, less at high frequencies
        ...
        spectral_filter = freqs ** (-0.5)  # -1.0/2 for power spectrum (1/f characteristic)
        
        # Apply spectral shaping
        fft_shaped = fft_white * spectral_filter
        
        # Back to time domain
        emg = np.fft.irfft(fft_shaped, n=len(signal))
        
        # Bandpass filter to EMG range (20-500 Hz)
        # Now with 1/f characteristics, high frequencies have less power
    ...
    def add_amplifier_noise(self,
        ...
        signal : np.ndarray
            Original signal
        amplitude : float
            RMS amplitude in mV (typically 1-5 Ã‚ÂµV)
    ...
    def add_composite_noise(self,
        ...
        # CRITICAL FIX: Replace flat white noise with frequency-shaped noise
        # Real physiological noise has 1/f characteristics, not flat spectrum
        noisy = self.add_physiological_broadband_noise(noisy, snr_db=snr_db)
        
        # Add amplifier noise (very small, high-frequency component)
        noisy = self.add_amplifier_noise(noisy, amplitude=0.003)
        
        return noisy
    
    def add_physiological_broadband_noise(self,
                                          signal: np.ndarray,
                                          snr_db: float = 10.0) -> np.ndarray:
        """
        Add physiologically realistic broadband noise with 1/f^α characteristics.
        
        Unlike flat white noise, real biological recordings show power that
        decreases with frequency. This creates a more realistic noise profile.
        ...
        --------
        noisy_signal : np.ndarray
            Signal with physiologically realistic broadband noise
        """
        # Calculate desired noise power from SNR
        signal_power = np.mean(signal[signal != 0] ** 2)
        snr_linear = 10 ** (snr_db / 10)
        ...
        # Apply 1/f^α spectral shaping
        # α = 1.0 gives pink noise (1/f)
        # α = 0.7-0.8 is typical for biological signals
        fft_white = np.fft.rfft(white)
        freqs = np.fft.rfftfreq(len(signal), 1/self.sampling_rate)
        ...
        # Power falls off as 1/f^α where α ≈ 0.8 (realistic for EMG/EEG)
        alpha = 0.8
        spectral_filter = freqs ** (-alpha / 2.0)  # /2 because we're filtering amplitude, not power
        
        # Apply shaping
        fft_shaped = fft_white * spectral_filter
        
        # Back to time domain
        colored_noise = np.fft.irfft(fft_shaped, n=len(signal))
        
        # Scale to achieve target noise power
        current_noise_power = np.mean(colored_noise ** 2)
        scaling_factor = np.sqrt(target_noise_power / current_noise_power)
        colored_noise = colored_noise * scaling_factor
